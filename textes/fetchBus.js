...
import templates from '../templates/Bus.js'
...
export default async (event) => {
  try {
    ...
    $('.traffic-option').each(function () {
      ...
      bubble.body.contents[0].text = $(this).find('.card-title').eq(1).text()
      bubble.body.contents[1].contents[0].contents[1].text = $(this).find('.card-text-info').eq(1).find('li').text()
